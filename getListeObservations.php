...
if (isset($_POST['lat']) && isset($_POST['lng']) && isset($_POST['page']) && isset($_POST['tri']) && isset($_POST['id_user'])) {
	...
	if ($_POST['tri'] == "date") {
		$orderBy = "ORDER BY publidate DESC";
	}
	else if ($_POST['tri'] == "distance") {
	...
	$reponse = $bdd -> prepare("SELECT 	`observations`.id AS id_obs, 
		...
		" " . $orderBy . 
		" LIMIT :debut , 20;");
	$reponse -> bindValue(':lat', $_POST['lat']);
	$reponse -> bindValue(':lng', $_POST['lng']);
	$reponse -> bindValue(':debut', ($_POST['page']-1)*20, PDO::PARAM_INT);
	if (isset($_POST['id_user'])) {
		$reponse -> bindValue(':id_user', $_POST['id_user']);
	}
	$reponse -> execute();
